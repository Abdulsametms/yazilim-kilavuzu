...
def register():
  ...
  if request.method == 'POST':
    ...
    mevcut = AdminUser.query.filter_by(username=kullanici_adi).first()
    if mevcut:
      ...
      db.session.commit()
      basari = (
          f'Kayıt başarılı! Özel Anahtarınız (Private Key):'
          f' {generated_private_key}'
      )

  return render_template('admin/register.html', hata=hata, basari=basari)


# Yönetim Giriş Rotası (Matematiksel Captcha ile)
...
def yonetim():
  ...
  if request.method == 'POST':
    ...
    if cevap_int != session.get('math_answer'):
      ...
      if admin:
        if admin.is_banned:
          hata = 'Bu hesap sistem yöneticisi tarafından banlanmıştır!'
        else:
...
def dashboard():
  ...
  if request.method == 'POST':
    ...
    if islem == 'urun_ekle':
      ...
      if aktif_rol != 'Admin':
        hata = "Yeni kullanıcı ekleme yetkiniz yok! Yalnızca 'Admin' yapabilir."
      else:
        yeni_username = request.form.get('username')
        mevcut_mu = AdminUser.query.filter_by(username=yeni_username).first()
        if mevcut_mu:
          ...
          db.session.commit()
          basari = 'Yeni kullanıcı ve private key başarıyla oluşturuldu.'
      ...
      if aktif_rol != 'Admin':
        ...
        if hedef:
          hedef.role = yeni_rol
          db.session.commit()
          basari = f"{hedef.username} kullanıcısının rolü '{yeni_rol}' yapıldı."
      ...
      if aktif_rol != 'Admin':
        ...
        if hedef and hedef.username != aktif_kullanici_adi:
          ...
          db.session.commit()
          durum = 'banlandı' if hedef.is_banned else 'banı kaldırıldı'
